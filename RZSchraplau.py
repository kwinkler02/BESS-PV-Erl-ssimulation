...
def solve_joint(prices, pv_vecs, ev_cons, cfgs, grid_kw, interval_h, progress_callback=None):
    # Indizes dynamisch
    su_idx = next((i for i,c in enumerate(cfgs) if c['mode']=='Single Use'), None)
    mu_idx = next((i for i,c in enumerate(cfgs) if c['mode']=='Multi Use'), None)
    ...
    for t in range(T):
        ...
        # Netzlimit: SU zuerst
        pv_to_ev=0
        if mu_idx is not None: pv_to_ev=min(pv_vecs[mu_idx][t],ev_cons[t])
        pv_surplus = pv_vecs[mu_idx or 0][t]-pv_to_ev
        su_flow=0
        if su_idx is not None: su_flow=dh_vars[su_idx][t]-ch_vars[su_idx][t]
        m += pv_surplus + max(su_flow,0) <= grid_max
        mu_flow=0
        if mu_idx is not None: mu_flow=dh_vars[mu_idx][t]-ch_vars[mu_idx][t]-max(ev_cons[t]-pv_to_ev,0)
        m += pv_surplus + max(su_flow,0) + max(mu_flow,0) <= grid_max
        if progress_callback and t%(max(1,T//50))==0: progress_callback(5+int(45*t/T))
    # Zyklenlimit
    for i in range(n):
        m += pulp.lpSum((ch_vars[i][tk]+dh_vars[i][tk])/(2*caps[i]) for tk in range(T)) <= max_cycs[i]
    if progress_callback: progress_callback(50)
    pulp.PULP_CBC_CMD(msg=False,timeLimit=180).solve(m)
    if progress_callback: progress_callback(90)
    obj = pulp.value(m.objective) or 0.0
...
def run_sim():
    ...
    free_res=[]
    for cfg, vec in zip(configs,[pv,pv] if enable2 else [pv]):
        free_res.append(solve_battery(prices,vec,cfg,grid_kw,interval_h,set_progress))
    joint_res = solve_joint(prices,[pv,pv] if enable2 else [pv], ev, configs, grid_kw, interval_h, set_progress)
...
for i,cfg in enumerate(configs,start=1): ind=float(np.dot(prices,joint_dhs[i-1]-joint_chs[i-1])); st.metric(f"B{i} Joint",fmt_euro(ind))
...
# Detaillierung (MU/SU) & GridLoad hier ggf. ergänzen...
